...
\begin{table}[H]
\centering
\caption{Classical ML model comparison — corrected 5-feature set, study dataset.}
\label{tab:classical_results}
\begin{tabular}{lrrrr}
\toprule
\textbf{Model} & \textbf{Accuracy} & \textbf{F1 Macro} & \textbf{AUC} & \textbf{Training time} \\
\midrule
Random Forest  & 89.6\% & 89.5\% & 0.989 & 0.4\,s \\
SVM (RBF)      & 80.8\% & 80.7\% & 0.964 & 0.1\,s \\
MLP            & 77.9\% & 77.6\% & 0.948 & 0.2\,s \\
GBT            & \textbf{90.0\%} & \textbf{90.0\%} & 0.987 & 1.6\,s \\
\bottomrule
\end{tabular}
\end{table}
...
Tanker (80) misclassification.
Table~\ref{tab:cm_gbt} shows the GBT confusion matrix as the best-performing model.
...
The authors would like to thank the radar system operators and data custodians for providing the \texttt{radarfeatureL\_Study} dataset. This work was supported in part by [Funding Body/Grant].
