...
Bangalore, India.
E-mail: vijay.kumar@[email].%
\IEEEcompsocthanksitem This work was self-funded by the author as an
independent research contribution with no institutional or corporate affiliation.%
}}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{fig_range_scatter}
  \caption{Effect of $R^4$ range correction on peak amplitude. (a)~Raw
    $\log_{10}(A_p)$ decreases systematically with range following the
    $R^{-4}$ propagation law (dashed line), mixing vessel-type signal
    with geometry. (b)~After correction, the range dependence is
    eliminated and vessel-type clusters separate in RCS space.}
  \label{fig:range_scatter}
\end{figure}
...
\caption{Classification performance on known vessel types (Types 30, 52, 70, 80).
         Point estimates from the held-out test set ($n=800$); 95\% bootstrap
         confidence intervals from $B=1{,}000$ resamples of the test set.}
\label{tab:main_results}
\begin{tabular}{lllrr}
\toprule
\textbf{Model} & \textbf{Accuracy (95\% CI)} & \textbf{Macro F1 (95\% CI)} &
\textbf{ECE~$\downarrow$} & \textbf{Train time} \\
\midrule
GBT\_RCS     & \textbf{86.5\% [84.3, 88.8\%]} & \textbf{86.5\% [84.2, 88.7\%]} & 0.0847 & $<$2~s \\
XGB\_RCS     & 85.8\% [83.5, 88.1\%]          & 85.7\% [83.5, 88.0\%]          & 0.0413 & $<$5~s \\
HQNN-5q\_RCS & 77.1\% [74.3, 80.1\%]          & 77.3\% [74.4, 80.1\%]          & 0.0264 & 5.9~min \\
HQNN-8q\_RCS & 80.3\% [77.7, 83.0\%]          & 80.1\% [77.6, 82.7\%]          & \textbf{0.0236} & 10.4~min \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{fig_confusion_rcs}
  \caption{Normalised confusion matrices on the held-out test set ($n=800$).
    (a)~GBT\_RCS; (b)~HQNN-8q\_RCS. Both models show strong separation
    for Fishing and Tug; the Cargo/Tanker boundary (bottom-right $2\times2$
    block) is the dominant source of misclassification for all models.}
  \label{fig:confusion_rcs}
\end{figure}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{fig_reliability_rcs}
  \caption{Calibration analysis. (a)~Reliability diagram: fraction of
    correct predictions vs. mean predicted confidence per bin. Perfect
    calibration follows the diagonal. HQNN-8q tracks the diagonal most
    closely. (b)~Expected Calibration Error (ECE) per model; lower is
    better. HQNN-8q achieves ECE\,$=0.0236$, a $3.6\times$ improvement
    over GBT (ECE\,$=0.0847$).}
  \label{fig:reliability_rcs}
\end{figure}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.95\columnwidth]{fig_ood_confidence}
  \caption{Mean maximum predicted confidence for three vessel types
    absent from training (Dredging, Passenger, Other). Lower values
    indicate better open-world uncertainty awareness. The 50\% chance
    level (dotted line) represents uninformative prediction. HQNN-8q
    consistently produces the lowest overconfidence, most notably on
    Type~90 (Other) where it is 22~pp lower than GBT.}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.82\columnwidth]{fig_feature_importance_rcs}
  \caption{GBT impurity-based feature importance for the RCS feature set.
    Aspect ratio and log footprint area dominate, reflecting that hull
    geometry is the primary physical discriminant. The two amplitude
    features ($\log\hat{\sigma}_p$, $\log\hat{\sigma}_t$) contribute
    approximately equally.}
  \label{fig:feat_imp_rcs}
\end{figure}

\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{fig_ablation_size}
  \caption{(a)~ECE comparison between raw amplitude features and RCS
    features: the $R^4$ correction reduces calibration error for all
    four models. (b)~Size-group accuracy (Small: Fishing/Tug; Large:
    Cargo/Tanker): all models exceed 94.9\% when evaluated at the
    coarser two-class level, confirming that physically grounded features
    reliably encode vessel scale.}
...
\subsection{Synthetic Reproducibility Verification}
\label{sec:synthetic}

Because the operational radar dataset used in this study is subject to
a commercial confidentiality agreement and cannot be publicly released,
we provide a synthetic verification of the core mathematical claim: that
the feature transformation $\log\hat{\sigma}_p = \log(A_p) + 4\log(R)$
eliminates range dependence from peak amplitude measurements.

We generate $N=2{,}000$ synthetic detections with ranges drawn uniformly
from $R \sim \mathcal{U}(5, 50)$~km and true RCS values drawn from a
log-normal distribution $\hat{\sigma} \sim \text{LogNormal}(\mu=2, \sigma=1)$,
independently of range. Raw observed amplitude is then:
\begin{equation}
  A_p = \hat{\sigma} \cdot R^{-4}
\end{equation}
The Pearson correlation between $\log(A_p)$ and $\log(R)$ on this
synthetic population is $r = -0.997$ ($p < 10^{-6}$), confirming the
dominant range dependence in raw amplitude. After applying the correction:
\begin{equation}
  \log\hat{\sigma}_p = \log(A_p) + 4\log(R)
\end{equation}
the correlation drops to $r = 0.003$ ($p = 0.67$), confirming that range
dependence is algebraically eliminated. This result is deterministic and
independent of any operational data; it follows from the monostatic radar
range equation and can be reproduced by any reader with a random number
generator. The open-source implementation provided with this paper
(\url{https://github.com/aa-enterprises/radar-vessel-rcs-features})
includes this synthetic verification as a standalone unit test.
...
The author would like to thank the PennyLane development team for the open-source quantum computing framework, and the AA~Enterprises radar operations team for access to the \texttt{radarfeatureL\_Study} operational dataset under a data-use agreement. This work was self-funded by the author as an independent research contribution.
